...
TEST_F (saiL3RifTest, create_rif_on_zero_member_lag)
{
    ...
    sai_rc = sai_test_l3_create_bridge_port(lag_id, &bridge_port_id);
    ASSERT_EQ (SAI_STATUS_SUCCESS, sai_rc);
    ...
    /* Remove the LAG */
    sai_rc = sai_test_l3_remove_bridge_port(bridge_port_id);
    EXPECT_EQ(SAI_STATUS_SUCCESS, sai_rc);
...
TEST_F (saiL3RifTest, create_rif_on_active_lag)
{
    ...
    for (index = 0; index < member_count; index++) {
        ...
        member_arr [index] = sai_test_l3_add_lag_member (sai_lag_api_table,lag_id,
    ...
    for (index = 0; index < member_count; index++) {
        ...
        sai_rc = sai_test_l3_remove_lag_member(sai_lag_api_table,member_arr [index]);
    ...
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_test_add_port_to_vlan(switch_id,
                sai_vlan_api_table, sai_l3_default_vlan_obj_id_get(),
                port_arr[0]));
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_test_add_port_to_vlan(switch_id,
                sai_vlan_api_table, sai_l3_default_vlan_obj_id_get(),
...
TEST_F (saiL3RifTest, rif_lag_member_update)
{
    ...
    sai_rc = sai_test_l3_remove_lag_member (sai_lag_api_table,member_arr [1]);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);

    sai_rc = sai_test_l3_remove_lag_member (sai_lag_api_table,member_arr [3]);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);
    ...
    /* Remove the LAG members */
    sai_rc = sai_test_l3_remove_lag_member (sai_lag_api_table,member_arr [0]);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);

    sai_rc = sai_test_l3_remove_lag_member (sai_lag_api_table,member_arr [2]);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);
