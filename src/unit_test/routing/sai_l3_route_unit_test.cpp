...
TEST_F (saiL3RouteTest, route_next_hop_on_lag_id)
{
    ...
    ASSERT_EQ (SAI_STATUS_SUCCESS, sai_test_l3_create_bridge_port(lag_id, &bridge_port_id));
    for (index = 0; index < member_count; index++) {
        ...
        member_arr[index] = sai_test_l3_add_lag_member(sai_lag_api_table, lag_id, port_arr [index]);
    }
